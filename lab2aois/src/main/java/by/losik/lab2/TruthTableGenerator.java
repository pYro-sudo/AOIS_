...
public class TruthTableGenerator {
    ...
    public static void main(String[] args) {
        ...
        long decimalRepresentation = getDecimalRepresentation();
    ...
    public static boolean evaluate(String expr, Map<String, Boolean> values) {
        expr = expr.replaceAll("\\s+", "");
        return evaluateExpression(expr, values);
    }

    public static boolean evaluateExpression(String expr, Map<String, Boolean> values) {
        if (expr.isEmpty()) {
            throw new IllegalArgumentException("Пустое выражение");
        }

        // Обработка вложенных скобок
        while (expr.startsWith("(") && expr.endsWith(")") && isEnclosedInParentheses(expr)) {
    ...
    private static boolean isEnclosedInParentheses(String expr) {
        int parenCount = 0;
        for (int i = 0; i < expr.length(); i++) {
            char c = expr.charAt(i);
            if (c == '(') parenCount++;
    ...
    public static int getOperatorPriority(String expr, int pos) {
    ...
    public static String getOperator(String expr, int pos) {
        if (pos + 1 < expr.length()) {
            String twoCharOp = expr.substring(pos, pos + 2);
    ...
    public static String buildSDNF() {
    ...
    public static String buildSKNF() {
    ...
    public static String getFunctionVector() {
    ...
    public static long getDecimalRepresentation() {
        String vector = getFunctionVector();
        return Long.parseLong(vector, 2);
    }

    public static String buildIndexSDNF() {
    ...
    public static String buildIndexSKNF() {
        List<Integer> indices = new ArrayList<>();
    ...
    public static String joinIndices(List<Integer> indices) {
    ...
    public void injectExpression(String expression) {
        TruthTableGenerator.expression = expression;
    }
}